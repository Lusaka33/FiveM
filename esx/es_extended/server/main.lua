        ...
        function(inventory)
          ...
          for i=1, #inventory, 1 do
            table.insert(userData.inventory, {
              ...
              ppi       = ESX.Items[inventory[i].item].ppi,
              poid      = ESX.Items[inventory[i].item].ppi * inventory[i].count,
...
AddEventHandler('esx:giveInventoryItem', function(target, type, itemName, itemCount, ammotogive)
  ...
  if type == 'item_standard' then
    ...
    if itemCount > 0 and sourceItem.count >= itemCount then
      ...
      if targetXPlayer.getInvPoid() + (itemCount * targetItem.ppi) <= Config.InvPoid then
        ...
        TriggerClientEvent('esx:showNotification', target, "Vous n'avez pas assez de place")
      end
    else
...
AddEventHandler('esx:onPickup', function(id)
  ...
  if pickup.type == 'item_standard' then
    ...
    if xPlayer.getInvPoid() + (item.count * item.ppi) <= Config.InvPoid then
