function CreateExtendedPlayer(player, accounts, inventory, job, loadout, name, lastPosition)
  ...
  self.getInvPoid = function()
    local temp = 0
    for k,v in pairs(self.inventory) do
      temp = temp + (v.ppi * v.count)
    end
    for k,v in pairs(self.loadout) do
      temp = temp + (v.ppi or 0)
    end
    return temp
  end
  ...
  self.addInventoryItem = function(name, count)
    ...
    local restant  = self.getInvPoid()
    if restant + (item.ppi * count) <= Config.InvPoid then
      ...
      TriggerClientEvent('esx:showNotification', self.source, 'Vous n\'avez pas assez de place dans votre inventaire')
    end
  end
  ...
  self.removeInventoryItem = function(name, count)
    ...
    if newCount >= 0 then
      ...
      TriggerClientEvent('esx:showNotification', self.source, 'Vous n\'avez pas assez d\'item dans votre inventaire')
    end
  end
  ...
  self.addWeapon = function(weaponName, ammo)
    ...
    for i=1, #Config.Weapons, 1 do
      if Config.Weapons[i].name == weaponName then
        ...
        weaponPpi = Config.Weapons[i].ppi
        break
      end
    end
    ...
    if (self.getInvPoid() + weaponPpi) <= Config.InvPoid then
