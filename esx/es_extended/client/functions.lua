...
ESX.ShowInventory = function()
  ...
  for i=1, #ESX.PlayerData.inventory, 1 do
    ...
    if ESX.PlayerData.inventory[i].count > 0 then
      table.insert(elements, {
        label     = ESX.PlayerData.inventory[i].label .. ' x' .. ESX.PlayerData.inventory[i].count .. ' [ ' .. ((ESX.PlayerData.inventory[i].ppi / 10) * ESX.PlayerData.inventory[i].count) .. 'kg ]',
        count     = ESX.PlayerData.inventory[i].count,
        type      = 'item_standard',
        value     = ESX.PlayerData.inventory[i].name,
        usable    = ESX.PlayerData.inventory[i].usable,
        ppi       = ESX.PlayerData.inventory[i].ppi,
        rare      = ESX.PlayerData.inventory[i].rare,
        canRemove = ESX.PlayerData.inventory[i].canRemove,
      })
      poidT = poidT + (ESX.PlayerData.inventory[i].ppi * ESX.PlayerData.inventory[i].count)
    end
  end
  ...
  for i=1, #Config.Weapons, 1 do
    ...
    if HasPedGotWeapon(playerPed,  weaponHash,  false) and Config.Weapons[i].name ~= 'WEAPON_UNARMED' then
      ...
      table.insert(elements, {
        label     = Config.Weapons[i].label .. ' (munition : ' .. ammo .. ') [' .. (Config.Weapons[i].ppi / 10) .. "kg]",
        count     = 1,
        type      = 'item_weapon',
      ...
      poidT = poidT + Config.Weapons[i].ppi
    end
  end
