...
AddEventHandler('esx:playerLoaded', function(xPlayer)
  ...
  for i=1, #xPlayer.accounts, 1 do
    ...
    local accountTpl = '<div style="color:rgb(255,0,0)"><img src="img/accounts/' .. xPlayer.accounts[i].name .. '.png"/>&nbsp;{{money}}</div>'
  ...
  local jobTpl = '<div style="padding-top:30px">{{job_label}} - {{grade_label}}</div>'
...
AddEventHandler('esx:addInventoryItem', function(item, count)
  ...
  if ESX.UI.Menu.IsOpen('inventory', 'es_extended', 'inventory') then
    ESX.ShowInventory()
  end
end)
...
Citizen.CreateThread(function()
  while true do
    ...
    if IsControlPressed(0, Keys["K"]) and not ESX.UI.Menu.IsOpen('inventory', 'es_extended', 'inventory') and (GetGameTimer() - GUI.Time) > 150 then
      ESX.ShowInventory()
