...
AddEventHandler('esx_holdupbank:toofar', function(robb)
	...
	for i=1, #xPlayers, 1 do
 		...
 		if xPlayer.job.name == 'police' or xPlayer.job.name == 'sherif' then
	...
	if Banks[robb] then
		...
		if rob == false then
			...
			if(cops >= Config.NumberOfCopsRequired)then
				...
				TriggerClientEvent('police:callBraquage', source, bank.position)
        TriggerClientEvent('sheriff:callBraquage', source, bank.position)
				...
				TriggerClientEvent('esx:showNotification', source, _U('min_two_police') .. Config.NumberOfCopsRequired .. _U('in_city2'))
			end
		else
