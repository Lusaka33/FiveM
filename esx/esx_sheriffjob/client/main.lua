...
RegisterNetEvent('sheriff:updatecall')
AddEventHandler('sheriff:updatecall', function(list)
  Appels = list
  if mycall > 0 then
    local isExist = false
...
RegisterNetEvent('sheriff:newcall')
AddEventHandler('sheriff:newcall', function(toogle)
  if in_service == true then
    ESX.ShowNotification('17 : Nouvel Appel !')
    SendNUIMessage({ action = "play_bip" })
  end
end)

RegisterNetEvent('sheriff:callBraquage')
AddEventHandler('sheriff:callBraquage', function(pos)
    pos['z'] = pos['z'] + 100
    TriggerServerEvent('phone:call', 'sheriff', "Auto_braquage", pos['x'], pos['y'], pos['z'])
end)
...
function OpenCloakroomMenu()
  ...
  table.insert(elements, {label = _U('rem_bullet_wear'), value = 'rem_bullet_wear'})
  table.insert(elements, {label = _U('rem_gilet_wear'), value = 'rem_gilet_wear'})
    ...
    function(data, menu)
      ...
      if data.current.value == 'cadet_wear' then
        ESX.TriggerServerCallback('esx_service:enableService', function(toogle, nb)
          if toogle == true then
            ESX.ShowNotification("Vous êtes maintenant en service avec " .. nb .. " collegues")
            in_service = true
...
function OpenVehicleSpawnerMenu(station, partNum)
  ...
  if Config.EnableSocietyOwnedVehicles then
    ...
    if PlayerData.job.grade_name == 'recruit' then
      table.insert(elements, { label = 'LEXUS', value = 'sheriff2'})
    end

    if PlayerData.job.grade_name == 'officer' then
	    table.insert(elements, { label = 'LEXUS', value = 'sheriff2'})
	    table.insert(elements, { label = 'AMG', value = 'sheriff'})
      table.insert(elements, { label = 'Van sheriff', value = 'sherifft'})
    end
    ...
    if PlayerData.job.grade_name == 'sergeant' then
	    ...
	    table.insert(elements, { label = 'AMG', value = 'sheriff'})
      table.insert(elements, { label = 'Van sheriff', value = 'sherifft'})
      table.insert(elements, { label = 'Interceptor', value = 'sheriff13'})
      table.insert(elements, { label = 'Banalisé', value = 'unmarked9'})
      table.insert(elements, { label = 'Moto', value = 'sheriffb'})
    end
    ...
    if PlayerData.job.grade_name == 'lieutenant' then
      ...
      table.insert(elements, { label = 'Ferrari', value = 'polf430'})
    end
    ...
    if PlayerData.job.grade_name == 'boss' then
      ...
      table.insert(elements, { label = 'Antiémeute', value = 'riot'})
      table.insert(elements, { label = 'Ferrari', value = 'polf430'})
	    table.insert(elements, { label = 'Chiron', value = 'polchiron'})
    end
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        if not DoesEntityExist(vehicle) then
          ...
          if Config.Maxin_service == -1 then
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        if data.current.value == 'openInter' then
          ...
          ESX.UI.Menu.Open(
            'default', GetCurrentResourceName(), 'sheriff_inter',
            {
            title    = "Interventions",
            align    = 'top-left',
            elements = els,
            ...
            function(data2, menu2)
              ...
              if data2.current.value == 'close' then
                TriggerServerEvent('sheriff:endcall', mycall)
                if raison ~= "Auto_braquage" then
                  TriggerServerEvent('sheriff:sendNotif', mycall, "Votre appel à était terminé avec succès")
                end
                mycall = 0
                raison = ""
                OpenSheriffActionsMenu()
              elseif data2.current.value == 'retour' then
                OpenSheriffActionsMenu()
              elseif data2.current.value == 'passez' then
                if raison ~= "Auto_braquage" then
                  TriggerServerEvent('sheriff:sendNotif', mycall, "Le Poulet a fait demi-tour, il va en pause donuts et doit être remplacer par un collégue")
                end
                raison = ""
                mycall = 0
                OpenSheriffActionsMenu()
              else
                if data2.current.rs ~= "Auto_braquage" then
                  TriggerServerEvent('sheriff:sendNotif', data2.current.value, "Un policier se rend sur votre appel, veuillez attendre son arrivée")
                end
                raison = data2.current.rs
                mycall = data2.current.value
                SetNewWaypoint(data2.current.x, data2.current.y)
                OpenSheriffActionsMenu()
              end
            end,
            function(data2, menu2)
        ...
        if data.current.value == 'citizen_interaction' then
          ...
          local els = {
            ...
            {label = 'Retirer un permis',           value = 'remove_license'},
            ...
            {label = "Mettre en prison",            value = 'jail'},
            {label = "Montrer le badge : Sheriff",  value = 'badge1'},
            {label = "Montrer le badge : BAC",      value = 'badge2'}
          }
          if PlayerData.job.grade_name == 'boss' or PlayerData.job.grade_name == 'lieutenant' then
            table.insert(els, {label = "Alerte à la population", value = 'alert'})
          end
          ESX.UI.Menu.Open(
            ...
            function(data2, menu2)
              ...
              if data2.current.value == 'alert' then
                ESX.UI.Menu.Open(
                  'default', GetCurrentResourceName(), 'Sheriff_alert',
                  {
                    title    = "Type d'alerte :",
                    align    = 'top-left',
                    elements = {
                      {label = "Recherche de personne", type = 'search'},
                      {label = "Alerte Attentat",       type = 'attentat'},
                      {label = "Code rouge",            type = 'red'},
                      {label = "Alerte custom",         type = 'custom'}
                    },
                  },
              ...
              if data2.current.value == 'remove_license' then
                ESX.UI.Menu.Open(
                  'default', GetCurrentResourceName(), 'Sheriff_alert',
                  {
                    title    = "Type d'alerte :",
                  ...
                  function(data3, menu3)
                    ...
                    if distance ~= -1 and distance <= 3.0 then
                      ...
                      if data2.current.value == 'license_weapon_remove' then
                        TriggerServerEvent('esx_sheriffjob:deletelicense', GetPlayerServerId(player), 'weapon')
                      end

                      if data2.current.value == 'license_moto_remove' then
                        TriggerServerEvent('esx_sheriffjob:deletelicense', GetPlayerServerId(player), 'drive_bike')
                      end
                      if data2.current.value == 'license_camion_remove' then
                        TriggerServerEvent('esx_sheriffjob:deletelicense', GetPlayerServerId(player), 'drive_truck')
                      end
                      if data2.current.value == 'license_voiture_remove' then
                        TriggerServerEvent('esx_sheriffjob:deletelicense', GetPlayerServerId(player), 'drive')
                      end
                      if data2.current.value == 'license_code_remove' then
                        TriggerServerEvent('esx_sheriffjob:deletelicense', GetPlayerServerId(player), 'dmv')
                      end
                    else
                ...
                if distance ~= -1 and distance <= 3.0 then
                  ...
                  if data2.current.value == 'license_see' then
                    TriggerServerEvent('esx_sheriffjob:license_see', GetPlayerServerId(player))
                  end
                  ...
                  if data2.current.value == 'handcuff' then
                    TriggerServerEvent('esx_sheriffjob:handcuff', GetPlayerServerId(player))
                  end

                  if data2.current.value == 'drag' then
                    TriggerServerEvent('esx_sheriffjob:drag', GetPlayerServerId(player))
                  end

                  if data2.current.value == 'jail' then
                    menu2.close()
                    DisplayOnscreenKeyboard(1, "FMMC_MPM_NA", "", "(1 = 1 minute, 60 = 1 heure, 600 = 10h)", "", "", "", 180)
                    while (UpdateOnscreenKeyboard() == 0) do
                      DisableAllControlActions(0)
                      Wait(0)
                    end
                    if (GetOnscreenKeyboardResult()) then
                      TriggerServerEvent('Sheriff_putInJail', GetPlayerServerId(player), ((tonumber(GetOnscreenKeyboardResult()) or 1) * 60))
                    end
                  end

                  if data2.current.value == 'put_in_vehicle' then
                    TriggerServerEvent('esx_sheriffjob:putInVehicle', GetPlayerServerId(player))
                  end

                  if data2.current.value == 'out_the_vehicle' then
                    TriggerServerEvent('esx_sheriffjob:OutVehicle', GetPlayerServerId(player))
                  end

                  if data2.current.value == 'badge1' then
                    TriggerServerEvent("sheriff:showBadgeTo", GetPlayerServerId(player))
                    ESX.showNotification("Badge donner au joueur")
                  end

                  if data2.current.value == 'badge2' then
                    TriggerServerEvent("sheriff:showBadgeTo2", GetPlayerServerId(player))
                    ESX.showNotification("Badge donner au joueur")
                  end
...
function OpenIdentityCardMenu(player)
  ...
  if Config.EnableESXIdentity then
    ...
    ESX.TriggerServerCallback('esx_sheriffjob:getOtherPlayerData', function(data)
      ...
      local elements = {
        {label = "Steam : " .. data.name, value = nil},
        {label = _U('name') .. data.firstname .. " " .. data.lastname, value = nil},
        {label = jobLabel,  value = nil},
      }
...
function OpenBodySearchMenu(player)
  ...
  ESX.TriggerServerCallback('esx_sheriffjob:getOtherPlayerData', function(data)
    local elements = {}
    TriggerServerEvent('esx_sheriffjob:notify', GetPlayerServerId(player), '~o~Un agent de sheriff est en train de vous fouillez')
    local blackMoney = 0
...
function OpenFineCategoryMenu(player, category)
  ...
  ESX.TriggerServerCallback('esx_sheriffjob:getFineList', function(fines)
    ...
    for i=1, #fines, 1 do
      table.insert(elements, {
      label     = fines[i].label .. ' $' .. fines[i].amount .. ' + ' .. fines[i].permis .. ' pts',
      value     = fines[i].id,
      amount    = fines[i].amount,
      permis    = fines[i].permis,
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        local amount = data.current.amount
        local ptsP = data.current.permis
        ...
        --end
        TriggerServerEvent('esx_billing:sendForceBill', GetPlayerServerId(player), _U('fine_total') .. label, amount, "sheriff")
        TriggerServerEvent('permis:removePoints', GetPlayerServerId(player), ptsP)
  ...
  while true do
    ...
    if CurrentAction ~= nil then
      ...
      if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'sheriff' and (GetGameTimer() - GUI.Time) > 150 then
        ...
        if CurrentAction == 'menu_vehicle_spawner' and in_service then
          OpenVehicleSpawnerMenu(CurrentActionData.station, CurrentActionData.partNum)
        end

        if CurrentAction == 'menu_vehicle_spawner_heli' and in_service then
          if not IsAnyVehicleNearPoint(CurrentActionData.x, CurrentActionData.y, CurrentActionData.z,  3.0) then

            ESX.Game.SpawnVehicle('polmav', {
              x = CurrentActionData.x,
              y = CurrentActionData.y,
              z = CurrentActionData.z
            }, CurrentActionData.head, function(vehicle)
        ...
        if CurrentAction == 'delete_vehicle' and in_service then
          ...
          if Config.EnableSocietyOwnedVehicles then
            ...
            if
              ...
              GetEntityModel(vehicle) == GetHashKey('sheriff3') or
              GetEntityModel(vehicle) == GetHashKey('sheriff4') or
              GetEntityModel(vehicle) == GetHashKey('sheriffb') or
              GetEntityModel(vehicle) == GetHashKey('sherifft') or
              GetEntityModel(vehicle) == GetHashKey('polmav')
            then
              --TriggerServerEvent('esx_service:disableService', 'sheriff')
            end
          end
        ...
        if CurrentAction == 'menu_boss_actions' and in_service then
          ESX.UI.Menu.CloseAll()
        ...
        if CurrentAction == 'remove_entity' and in_service then
    ...
    if IsControlPressed(0,  Keys['F5']) and PlayerData.job ~= nil and PlayerData.job.name == 'sheriff' then
      OpenSheriffActionsMenu()
      GUI.Time = GetGameTimer()
    end
  end
...
-- NB : gestion des menu --
---------------------------
RegisterNetEvent('NB:openMenuSheriff')
AddEventHandler('NB:openMenuSheriff', function()
	OpenSheriffActionsMenu()
end)

RegisterNetEvent("esx_sheriffjob:notify")
AddEventHandler("esx_sheriffjob:notify", function(text)
