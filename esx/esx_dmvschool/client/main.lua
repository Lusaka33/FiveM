...
function StartDriveTest(type)
  ...
  ESX.Game.SpawnVehicle(Config.VehicleModels[type], Config.Zones.VehicleSpawnPoint.Pos, 317.0, function(vehicle)
    ESX.Game.SetVehicleProperties(vehicle, {plate = "AUTO" .. PLATE, health = 1000})
    CurrentTest       = 'drive'
...
function OpenDMVSchoolMenu()
  ...
  elseif ownedLicenses['dmv'] then

    if not ownedLicenses['paper'] then
      table.insert(elements, {label = "Aller chercher votre carnet à la préfécture : 200 $", value = 'carnet'})
    elseif ownedLicenses['paper'] then

      if not ownedLicenses['drive'] then
        table.insert(elements, {label = _U('road_test_car') .. Config.Prices['drive'], value = 'drive_test', type = 'drive'})
      elseif not ownedLicenses['drive_bike'] then
  ...
  ESX.UI.Menu.Open(
    ...
    function(data, menu)
      ...
      if data.current.value == 'theory_test' then
        menu.close()
        StartTheoryTest()
      elseif data.current.value == 'carnet' then
        menu.close()
        GoPrefecture()
...
function GoPrefecture()
  ESX.ShowNotification("Aller à la préfecture (le 'P' en rouge sur votre carte)")
  SetNewWaypoint(239.318, -405.901)
end

function OpenPrefectureMenu()
  ...
  if not ownedLicenses['dmv'] then
    table.insert(elements, {label = "Passer tout d'abord l'épreuve théorique à l'auto-école", value = 'theory_test'})
  elseif ownedLicenses['dmv'] then
    if not ownedLicenses['paper'] then
      table.insert(elements, {label = "Obtenir le Carnet: 200 $", value = 'carnet'})
    elseif ownedLicenses['paper'] then
      table.insert(elements, {label = "Vous avez déja votre carnet, aller maintenant passer l'épreuve pratique à l'auto-école", value = 'drive_test'})
    end
  end
  ...
  ESX.UI.Menu.Open(
    ...
    function(data, menu)
      ...
      if data.current.value == 'carnet' then
        menu.close()
        ESX.ShowNotification('Voici votre carnet, vous pouvez maintenant passer l\'épreuve pratique à l\'auto-école')
        TriggerServerEvent('esx_dmvschool:pay', 200)
        TriggerServerEvent('esx_dmvschool:addLicense', 'paper')
        GoPrefecture()
      end
    end,
    function(data, menu)
      menu.close()
      CurrentAction     = 'Prefecture_menu'
      CurrentActionMsg  = _U('press_open_menu')
      CurrentActionData = {}
    end
  )
...
Citizen.CreateThread(function()
  while true do
    ...
    if(IsPedInAnyVehicle(GetPlayerPed(-1), false)) and (not ownedLicenses['dmv'] or not ownedLicenses['drive']) and CurrentTest ~= 'drive' then
      DrawMissionText("~r~Vous conduisez sans permis, allez le passer!", 2000)
    end
  end
end)
...
Citizen.CreateThread(function()
  ...
  local blip2 = AddBlipForCoord(Config.Zones.Prefecture.Pos.x, Config.Zones.Prefecture.Pos.y, Config.Zones.Prefecture.Pos.z)
  ...
  while true do
    ...
    if CurrentAction ~= nil then
      ...
      if IsControlJustReleased(0,  Keys['E']) then
        ...
        if CurrentAction == 'Prefecture_menu' then
          OpenPrefectureMenu()
        end
